...
public class UserEntity extends BaseEntity {
    ...
    //UUID 전 세계적으로 유일한 식별자를 생성하는 방법, uuid라는 이름만으로는 UUID 생성 방식을 어떻게 처리해야할지 모르기 때문에 아래 코드를 추가로 기재해준 것
    @GenericGenerator(name = "uuid", strategy = "uuid")
    //Hibernate에서 제공, strategy = "uuid"는 Hibername가 기본적으로 제공하는 생성 방식
    @Column(insertable = false, updatable = false, nullable = false, length = 255)
    //인설터블 - 새로운 레코드가 삽입될 때 데이터베이스에 추가되지 않음, 업데이터블 - 기존 레코드가 수정될 때 변경되지 않음
    @Comment("고유 ID (UUID)")
    private String id;
